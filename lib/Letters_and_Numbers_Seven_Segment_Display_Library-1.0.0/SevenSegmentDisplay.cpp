...
void SevenSegmentDisplay::displayWord(String strsentence, int delaytime = 500){
  int a = strsentence.length();
  char sentence[a];
  strcpy(sentence, strsentence.c_str());
  for (int i = 0; i < a; i++){
    if (sentence[i] == ' '){
      clearDisplay();
    }else displayCharacter(sentence[i]);
    delay(delaytime);
  }
  clearDisplay();
}
